...
export class ClienteData extends BaseData{
  ...
  public async findClienteByEmail(email:string) : Promise<ClienteModel | undefined>{

    try {
       const cliente = await BaseData.connection(this.tableName).select("*")
        .where({email: email})
...
public async findClienteById(id_cliente:string) : Promise<ClienteModel | undefined>{

  try {
     const [cliente] : ClienteModel[]= await BaseData.connection(this.tableName).select("*")
...
public async findClienteByNome(nome_completo:string) : Promise<ClienteModel | undefined>{
...
public async getClientes () {
  try {
      const results = await ClienteData 
