...
export class ClienteBusiness {
  ...
  public async signup(clienteInput: ClienteInputDTO): Promise<string> {
    try {
      const { nome_completo, nome_social, email, senha, telefone } =
        clienteInput;
      ...
      const existingTelefone = await this.clienteData.findClienteByTelefone(
        telefone
      );
      if (existingCliente) {
  ...
  public async getClienteAndFormById(id_cliente: string, token: string) {
    try {
      ...
      const formulario = await this.formularioData.findFormularioByUserId(
        id_cliente
      );
